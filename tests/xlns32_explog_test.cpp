// Test file for xlns32 exp, log, exp2, log2, pow, and softmax functions
...
void test_exp() {
    printf("--- xlns32_exp(x) vs expf(x) ---\n");
    printf("  x       got           expected      err%%\n");
    float xs[] = {-4.0f, -2.0f, -1.0f, -0.5f, 0.0f, 0.5f, 1.0f, 2.0f, 4.0f};
    for (float x : xs) {
        xlns32 r = xlns32_exp(fp2xlns32(x));
        float got = xlns322fp(r);
...
void test_log() {
    printf("--- xlns32_log(x) vs logf(x) ---\n");
    printf("  x       got           expected      err%%\n");
    float xs[] = {0.01f, 0.1f, 0.5f, 1.0f, 2.0f, 4.0f, 10.0f, 100.0f};
    for (float x : xs) {
        xlns32 r = xlns32_log(fp2xlns32(x));
        float got = xlns322fp(r);
        float log_val = logf(x);
        if (fabsf(log_val) < 1e-6f)
            printf("%6.2f  %12.7f  %12.7f  (exact zero)\n", x, got, log_val);
        else
            printf("%6.2f  %12.7f  %12.7f  %6.4f%%\n", x, got, log_val, relerr(got, log_val));
    }
    printf("\n");
}

void test_exp2() {
    printf("--- xlns32_exp2(x) vs exp2f(x) ---\n");
    printf("  x       got           expected      err%%\n");
    float xs[] = {-4.0f, -2.0f, -1.0f, -0.5f, 0.0f, 0.5f, 1.0f, 2.0f, 4.0f};
    for (float x : xs) {
        xlns32 r = xlns32_exp2(fp2xlns32(x));
        float got = xlns322fp(r);
...
void test_log2() {
    printf("--- xlns32_log2(x) vs log2f(x) ---\n");
    printf("  x       got           expected      err%%\n");
    float xs[] = {0.01f, 0.1f, 0.5f, 1.0f, 2.0f, 4.0f, 8.0f, 16.0f};
    for (float x : xs) {
        xlns32 r = xlns32_log2(fp2xlns32(x));
        float got = xlns322fp(r);
...
void test_pow() {
    printf("--- xlns32_pow(base, exp) vs powf(base, exp) ---\n");
    ...
    for (auto& c : cases) {
        xlns32 r = xlns32_pow(fp2xlns32(c.b), fp2xlns32(c.e));
        float got = xlns322fp(r);
        float pw = powf(c.b, c.e);
        printf("  %5.2f  %4.1f  %12.7f  %12.7f  %6.4f%%\n", c.b, c.e, got, pw, relerr(got, pw));
    }
    printf("\n");
}

void test_softmax_exp() {
    printf("--- xlns32_softmax_exp ---\n");
    printf("Input: [1, 2, 3, 4]\n");
    xlns32 in[4] = {fp2xlns32(1.0f), fp2xlns32(2.0f), fp2xlns32(3.0f), fp2xlns32(4.0f)};
    xlns32 out[4];
    xlns32_softmax_exp(in, out, 4);

    float expected[4] = {expf(-3.0f), expf(-2.0f), expf(-1.0f), expf(0.0f)};
    printf("  idx   got           expected      err%%\n");
    for (int i = 0; i < 4; i++) {
        float got = xlns322fp(out[i]);
        printf("  [%d]   %12.7f  %12.7f  %6.4f%%\n", i, got, expected[i], relerr(got, expected[i]));
    }
    printf("\n");
}

void test_softmax() {
    printf("--- xlns32_softmax ---\n");
    printf("Input: [1, 2, 3, 4]\n");
    xlns32 in[4] = {fp2xlns32(1.0f), fp2xlns32(2.0f), fp2xlns32(3.0f), fp2xlns32(4.0f)};
    xlns32 out[4];
    xlns32_softmax(in, out, 4);
...
int main() {
    printf("=============================================================\n");
    printf("  xlns32 exp / log / exp2 / log2 / pow / softmax test        \n");
